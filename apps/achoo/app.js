...
const state={nA:30,yA:1,nB:10,yB:3,k:5,model:"mackay"};
const pct=x=>(100*x).toFixed(1)+'%';
const prob=x=>x>.9995?'> 99.9%':x<.0005?'< 0.1%':pct(x);
const pval=x=>x===null?'—':x<.0001?'< 0.0001':x.toFixed(4);
function draw(id,points,color,label,overlay=[]){
 const W=420,H=238,L=43,R=12,T=18,B=40,lo=Math.min(points[0][0],overlay[0]?.[0]??Infinity),hi=Math.max(points.at(-1)[0],overlay.at(-1)?.[0]??-Infinity),max=Math.max(...points.map(p=>p[1]),...overlay.map(p=>p[1]))*1.1;
 ...
 if(overlay.length)svg+=`<path d="${path(overlay)}" fill="none" stroke="#575080" stroke-width="2.3" stroke-dasharray="7 4"/>`;
...
function update(){
 const {nA,yA,nB,yB,k}=state,N=nA+nB,A=S.beta(yA+1,nA-yA+1),B=S.beta(yB+1,nB-yB+1),f=S.frequentist(nA,yA,nB,yB);
 const brms=getBrms(nA,yA,nB,yB),mackay=[A,B].map(p=>({mean:p.a/(p.a+p.b),low:S.quantile(.025,p),high:S.quantile(.975,p)}));
 const mProbs=[S.probability(A,B,1),S.probability(A,B,k)],bProbs=brms.available?[brms.probability(1),brms.probability(k)]:[null,null];
 const selected=state.model==='mackay'?mackay:brms.available?brms.summaries:null,selectedProbs=state.model==='mackay'?mProbs:bProbs;
 $('model').value=state.model;for(const id of ['active-model-better','active-model-times'])$(id).textContent='Bayesian · '+(state.model==='mackay'?'MacKay':'brms defaults');$('summary-heading').textContent=(state.model==='mackay'?'MacKay':'brms')+' mean [95% CrI]';
 $('compare-k').textContent=`P(A ${k}× better | data)`;
 $('prior-comparison').innerHTML=[['MacKay · uniform risks',mProbs,mackay],['brms · logistic defaults',bProbs,brms.available?brms.summaries:null]].map(([name,ps,ss])=>`<tr><td>${name}</td><td>${ps[0]===null?'Unavailable':prob(ps[0])}</td><td>${ps[1]===null?'Unavailable':prob(ps[1])}</td><td>${ss?pct(ss[0].mean):'—'}</td><td>${ss?pct(ss[1].mean):'—'}</td></tr>`).join('');
 $('brms-note').textContent=!brms.available?brms.reason:brms.boundary?'brms probabilities and summaries use a checked numerical approximation. Its density overlay is omitted for boundary counts, where mass can concentrate extremely close to risk 0 or 1.':'brms values approximate the posterior under the R script’s default priors. Its flat coefficient prior is improper; it is not a uniform prior on disease risk.';
 ...
 $('counts').innerHTML=[['A',nA,yA,A,'a'],['B',nB,yB,B,'b']].map(([name,n,c,p,cls],i)=>`<tr><td><span class="dot ${cls}"></span>${name}</td><td>${c}</td><td>${n-c}</td><td>${n}</td><td>${pct(c/n)}</td><td>${selected?`${pct(selected[i].mean)} [${pct(selected[i].low)}, ${pct(selected[i].high)}]`:'Unavailable'}</td></tr>`).join('');
 ...
 $('test-warning').textContent=[f.warn?'Some expected counts are below 5; the χ² approximation may be unreliable.':'',f.z===null?'A zero cell makes the ordinary logistic coefficient/Wald test undefined (separation). MacKay’s Bayesian estimates remain defined.':''].filter(Boolean).join(' ');
 ...
 function overlay(multiplier){
  if(!brms.available||brms.boundary)return [];
  const [a,b]=brms.summaries,lo=b.quantile(.0005)-multiplier*a.quantile(.9995),hi=b.quantile(.9995)-multiplier*a.quantile(.0005);
  return brms.density(multiplier,Array.from({length:321},(_,i)=>lo+(hi-lo)*i/320));
 }
 draw('difference-plot',S.density(A,B,1),'#287e73','Posterior density of B minus A risk: MacKay solid and shaded above zero; brms dashed when available',overlay(1));
 draw('multiplier-plot',S.density(A,B,k),'#a14e6b',`Posterior density of B minus ${k} times A risk: MacKay solid and shaded above zero; brms dashed when available`,overlay(k));
}
...
$('model').addEventListener('change',e=>{state.model=e.target.value;schedule();});
$('k').addEventListener('input',e=>{state.k=Number(e.target.value);schedule();});
$('reset').addEventListener('click',()=>{Object.assign(state,{nA:30,yA:1,nB:10,yB:3,k:5,model:"mackay"});resizeRates=[1/30,3/10];update();});
update();
